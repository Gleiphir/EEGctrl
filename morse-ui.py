...
class mainWindow (QMainWindow):
    def __init__(self,joystick = False,fontsize=40):
        super().__init__()
    ...
    def MrsInput(self,sig:int):
        """ 0 = / ; 1 = . ; 2 = - ;  """
        if sig == 0:
            if not self.morseStr in MorseMapping:
                self.morseStr = ""
            else:
...
if __name__ == '__main__':
    ...
    mainW = mainWindow(joystick=True,fontsize= app.primaryScreen().size().height() // 12 )
    mainW.show()
    sys.exit(app.exec_())
