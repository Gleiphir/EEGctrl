...
class HoverableBtn(QPushButton):

    affirm = pyqtSignal(str)
...
class mainWindow (QMainWindow):
    ...
    def __init__(self,joystick = False):
        ...
        self.InputTimer.timeout.connect(self.inputDirect)
        self.currentChr = None
    ...
    @pyqtSlot(str)
    def newInput(self,t:str):
        self.currentChr = t
        self.InputTimer.start(FOCUS_TIME_S)

    def eventFilter(self, obj, event) -> bool:
        if obj in self.BtnLbls and event.type() == QEvent.HoverEnter:
            self.currentChr = obj.text()
            self.InputTimer.start(FOCUS_TIME_S * 1000)
    ...
    def inputDirect(self):
        if not self.currentChr:return
        #t = self.BtnLbls[self.areaCursor.cordIndex()].text()
        self.CM.select(self.currentChr)
        #print("t", t)
        #print("current",self.CM.getCurrent())
        #print("TEXT ", self.CM.textString)
        self.ShowText.setText(self.CM.textString)
